...
using FreelancerDirectoryAPI.Application.Common.Security;
using MediatR;
            ...
            if (authorizeAttributes.Any())
            {
                ...
                if (authorizeAttributesWithRoles.Any())
                {
                    foreach (var roles in authorizeAttributesWithRoles.Select(a => a.Roles.Split(',')))
                    {
                        ...
                        if (!authorized)
                        {
                            _logger.LogInformation("Match.FreelancerDirectoryAPI Authorization Request: {@UserId} {@Request}", _currentUserService.UserId, request);
